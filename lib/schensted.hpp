...
namespace Schensted
{
    namespace
    {
        ...
        size_t min_tails(RandomAccessIterator first, RandomAccessIterator last, ValueType* min_tails_buf, Compare comp)
        {
            ...
            for (; it != last; ++it) {
                if (comp(min_tails_buf[lis_len], *it)) {
                    min_tails_buf[++lis_len] = *it;
                }
                else {
                    auto pos = std::lower_bound(min_tails_buf + 1, min_tails_buf + lis_len + 1, *it, comp);
                    *pos = *it;
                }
            }
        ...
        size_t max_heads(RandomAccessIterator first, RandomAccessIterator last, ValueType* max_heads_buf, Compare comp)
        {
            ...
            auto greater_comp = [&comp](const ValueType& a, const ValueType& b) {
                return comp(b, a);
            };

            while (it != first) {
                --it;
                if (comp(*it, max_heads_buf[lis_len])) {
                    max_heads_buf[++lis_len] = *it;
                }
                else {
                    auto pos = std::lower_bound(max_heads_buf + 1, max_heads_buf + lis_len + 1, *it, greater_comp);
                    *pos = *it;
                }
            }
